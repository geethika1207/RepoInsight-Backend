import shutil  # <-- 1. ADD THIS IMPORT AT THE TOP
...
async def get_repository(repository_url: repository.RequestURL, db: AsyncSession = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    # --- 2. CLEAN UP OLD DIRECTORY IF IT ALREADY EXISTS ---
    ...
    # --- 3. WRAP PROCESSING IN TRY...FINALLY FOR AUTOMATIC DELETION ---
    try:
        # Save to database
        ...
        (
            ...
            build_report_query.report_query(repo_id, reports_queries[0], db),
            build_report_query.report_query(repo_id, reports_queries[1], db),
            build_report_query.report_query(repo_id, reports_queries[2], db),
            build_report_query.report_query(repo_id, reports_queries[4], db),
            build_report_query.report_query(repo_id, reports_queries[3], db),
            build_report_query.report_query(repo_id, reports_queries[9], db),
            build_report_query.report_query(repo_id, reports_queries[6], db),
            build_report_query.report_query(repo_id, reports_queries[7], db),
            build_report_query.report_query(repo_id, reports_queries[5], db),
            build_report_query.report_query(repo_id, reports_queries[8], db),
        ...
        # --- 4. CLEAN UP FOLDER AFTER RESPONSE OR ON ERROR ---
        if destination.exists():
            shutil.rmtree(destination, ignore_errors=True)